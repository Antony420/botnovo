...
async function starts() {
 ...
 ீ͜ৡৢ͜͡​😎BΣN VIND〄Sீ͜ৡৢ͜͡💜
Δ❍ ΣNTRΔR ΔPRΣSΣNTΣ-SΣ C❍M:

ஓீ͜🎡F❍T❍:
ஓீ͜🎡N❍MΣ:
ஓீ͜🎡IDΔDΣ:
ஓீ͜🎡CIDΔDΣ:
ஓீ͜🎡STATUS DE RELACIONAMENTO:
By ADMS E FAMILIA😎🌹
*╚════🎡+🎡+🎡+🎡════╝*️`
				...
				teks = ` saiu @${num.split('@')[0]} pq a Anaylle comeu 👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["558897903722@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://images.app.goo.gl/To7Qrc7dudXxFCD86g`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Antony\n* *WPP:* wa.me/+558897903722\n*INSTA:* to sem brow kk\n\n\n*'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'bot':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Informaçoes nao disponiveis,:( mas quer ter conhecimento em progamaçao procure o dono do bot.'})
					break
				case 'belle3':
					memein = await kagApi.memeindo()
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmm, O FBI esta de Olho em voce ;-;'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ' Mamou os ADMS e agora e um ADM tambem\n'
						...
						mentions(`Mamou os ADMS @${mentioned[0].split('@')[0]} E agora é admin então respeitem ok?! 😂`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Quer Adm? Vai ter que Mamar denovo! @${mentioned[0].split('@')[0]} Acabou de perder o ADM  😂!`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Mandioca passada com sucesso : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `OS DONOS DA QUEBRADA *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
